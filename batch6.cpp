...
public:
  ...
  void addPoint(double x, double y, int nSpline) {
    int posfound;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < dinp; i++) 
    for (int j = 0; j < trs; j++) 
      matXtr[i][j] = dataset.training_images[j][i] * 100;
  ...
  for (int i = 0; i < dinp; i++) 
    for (int j = 0; j < tes; j++) 
      matXte[i][j] = dataset.test_images[j][i] * 100;
  ...
  for (int q = 0; q < MNIST_NUM_CLASSES; q++) {
	...
	tpoints[j].y = (vecYtr[j] == q ? 1 : 0);
      ...
      for (int j = 0; j < trs; j++) {
	mxy->addPoint(tpoints[j].x, tpoints[j].y, i);
      }
